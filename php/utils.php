...
/**
 * Generate an HTML <time /> element for the designated datestamp.
 * 
...
/**
 * Generate an HTML tag with the supplied parameters.
 * 
...
/**
 * Split a line-delimited list of key:value pairs into an associative array.
 *
...
/**
 * Replace any variable declarations found in a string with the accompanying values found in the supplied key/value pair.
 * This provides a safer and more controlled alternative to PHP's native eval function.
 * 
 * @example tokenise('<input type="$type" id="$id" />', array(
 *    'type'  => 'text',
 *    'id'    => 'first-name
 * )); 
 */
...
/**
 * Check if user-supplied input logically equates to TRUE or FALSE.
 * 
...
/**
 * Remove indices from an array by key.
...
/**
 * Remove indices from an array based on their values.
 *
...
/** Return TRUE if a file appears to be of a valid image MIME type. */
...
/** Convert a local file path to an absolute URL. */
function path_to_url($input, $scheme = "http://"){
	return str_replace($_SERVER["DOCUMENT_ROOT"], $scheme . $_SERVER["HTTP_HOST"], $input);
}


/** Convert an absolute URL to a local filepath, assuming it points to a resource located on the server. */
...
/**
 * Output a boolean HTML attribute if $a tests equal to $b.
 *
...
/**
 * Return CSS-style shorthand for a collapsable hex sequence ("BBFFDD" => "BFD").
 *
...
/**
 * Return a compiled hexadecimal representation of one or more integers.
 *
...
/**
 * Format a number of bytes for human-readable format (KB, MB, GB).
 *
...
/**
 * Convert a string representing a properly formatted filesize (e.g., "25.25 MB") into its according number of bytes.
 *
...
/**
 * Normalise newlines in a string, replacing all Windows/Unix-style line-breaks with a consistent EOL terminator.
 *
...
/**
 * Return an array of all a class's subclasses, if any.
 * 
